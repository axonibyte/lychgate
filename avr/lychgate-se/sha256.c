...
{
    ctx->length_bits += (uint64_t)len * 8;
    while (len > 0) {
...
{
    uint64_t bits = ctx->length_bits;
    uint8_t pad = 0x80;
    uint8_t zero = 0x00;
    uint8_t len_be[8];
    int i;

    lg_sha256_update(ctx, &pad, 1);
    ctx->length_bits -= 8; /* padding does not count */
    while (ctx->buffered != 56) {
        lg_sha256_update(ctx, &zero, 1);
        ctx->length_bits -= 8;
    }
    for (i = 0; i < 8; i++)
        len_be[i] = (uint8_t)(bits >> (56 - 8 * i));
    lg_sha256_update(ctx, len_be, 8);
